...
/// MCP server exposing Markdotabase operations as tools for AI agents.
///
/// Holds a shared reference to the storage backend so all tool calls operate
/// on the same underlying data.
#[derive(Clone)]
pub struct MdbaseService {
...
impl fmt::Debug for MdbaseService {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MdbaseService")
...
#[tool_handler]
impl ServerHandler for MdbaseService {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            protocol_version: ProtocolVersion::V_2024_11_05,
            capabilities: ServerCapabilities::builder().enable_tools().build(),
            server_info: Implementation {
                name: "markdotabase".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
            },
            instructions: Some(
                "Markdotabase MCP server — a Markdown-oriented database for AI agents. \
...
pub async fn run_stdio(
    ...
    let service = match (index, graph, embedder, vector_index) {
        (Some(idx), Some(g), Some(emb), Some(vi)) => {
            MdbaseService::full(storage, idx, g, emb, vi)
        }
        _ => MdbaseService::new(storage),
    };
...
pub async fn run_http(
    ...
    let service_factory = {
        ...
        move || -> Result<MdbaseService, std::io::Error> {
            let service = match (&index, &graph, &embedder, &vector_index) {
                (Some(idx), Some(g), Some(emb), Some(vi)) => MdbaseService::full(
                    storage.clone(),
                    idx.clone(),
                    g.clone(),
                    emb.clone(),
                ...
                _ => MdbaseService::new(storage.clone()),
            };
            Ok(service)
        }
    };
