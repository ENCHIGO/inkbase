use inkbase_core::{BlockRecord, BlockType, InkbaseError};
...
///
/// Returns `InkbaseError::ParseError` if the YAML is malformed.
pub fn parse_frontmatter(yaml: &str) -> inkbase_core::Result<serde_json::Value> {
    let yaml_value: serde_yaml::Value =
        serde_yaml::from_str(yaml).map_err(|e| InkbaseError::ParseError(format!("invalid frontmatter YAML: {e}")))?;

    serde_json::to_value(yaml_value)
        .map_err(|e| InkbaseError::ParseError(format!("failed to convert YAML to JSON: {e}")))
}
