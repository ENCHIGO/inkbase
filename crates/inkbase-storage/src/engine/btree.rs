...
impl BTree {
    ...
    pub fn new(pool: Arc<BufferPool>) -> inkbase_core::Result<Self> {
    ...
    pub fn insert(&mut self, key: &[u8], value: &[u8]) -> inkbase_core::Result<()> {
    ...
    pub fn get(&self, key: &[u8]) -> inkbase_core::Result<Option<Vec<u8>>> {
    ...
    pub fn delete(&mut self, key: &[u8]) -> inkbase_core::Result<bool> {
        let leaf_id = self.find_leaf(key)?;
    ...
    ) -> inkbase_core::Result<Vec<(Vec<u8>, Vec<u8>)>> {
    ...
    pub fn prefix_scan(&self, prefix: &[u8]) -> inkbase_core::Result<Vec<(Vec<u8>, Vec<u8>)>> {
        if prefix.is_empty() {
    ...
    fn find_leaf(&self, key: &[u8]) -> inkbase_core::Result<PageId> {
    ...
    fn find_leftmost_leaf(&self) -> inkbase_core::Result<PageId> {
        let mut page_id = self.root_page_id;
