...
fn sled_err(err: sled::Error) -> MdbaseError {
    MdbaseError::StorageError(err.to_string())
}
...
impl SledStorage {
    ...
    pub fn new(path: &Path) -> mdbase_core::Result<Self> {
    ...
    fn serialize<T: serde::Serialize>(value: &T) -> mdbase_core::Result<Vec<u8>> {
    ...
    fn deserialize<T: serde::de::DeserializeOwned>(bytes: &[u8]) -> mdbase_core::Result<T> {
...
impl Storage for SledStorage {
    ...
    fn get_document_by_path(&self, path: &str) -> mdbase_core::Result<Option<DocumentRecord>> {
        ...
        let doc_id = Uuid::from_slice(&id_bytes)
            .map_err(|e| MdbaseError::StorageError(format!("corrupt doc_id in path index: {e}")))?;
...
mod tests {
    use super::*;
    use chrono::Utc;
    use mdbase_core::types::{BlockType, LinkType};
    use uuid::Uuid;
