...
pub async fn ingest_document(
    ...
    // Parse markdown.
    let parse_result = inkbase_parser::parse_markdown(&req.path, &req.content).map_err(|e| {
    ...
    tokio::task::spawn_blocking(move || -> inkbase_core::Result<()> {
...
pub async fn get_document(
    ...
    let (doc, tags) = tokio::task::spawn_blocking(move || -> inkbase_core::Result<_> {
        let doc = storage
            .get_document_by_path(&lookup_path)?
            .ok_or_else(|| {
                inkbase_core::InkbaseError::DocumentNotFound(lookup_path.clone())
            })?;
    ...
    .map_err(join_error)?
    .map_err(|e| match &e {
        inkbase_core::InkbaseError::DocumentNotFound(_) => (
...
pub async fn delete_document(
    ...
    let doc_id = tokio::task::spawn_blocking(move || -> inkbase_core::Result<_> {
        let doc = storage
            .get_document_by_path(&lookup_path)?
            .ok_or_else(|| {
                inkbase_core::InkbaseError::DocumentNotFound(lookup_path.clone())
...
pub async fn get_blocks(
    ...
    let blocks = tokio::task::spawn_blocking(move || -> inkbase_core::Result<_> {
        let doc = storage.get_document_by_path(&path)?.ok_or_else(|| {
            inkbase_core::InkbaseError::DocumentNotFound(path.clone())
        })?;
