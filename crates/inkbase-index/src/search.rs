...
use inkbase_core::types::{BlockRecord, DocumentRecord};
...
impl FullTextIndex {
    ...
    pub fn new(index_dir: &Path) -> inkbase_core::Result<Self> {
        ...
        std::fs::create_dir_all(index_dir).map_err(|e| {
            InkbaseError::IndexError(format!("failed to create index directory: {e}"))
        })?;

        let index = Index::open_or_create(
            tantivy::directory::MmapDirectory::open(index_dir)
                .map_err(|e| InkbaseError::IndexError(e.to_string()))?,
            schema,
        )
        .map_err(|e| InkbaseError::IndexError(e.to_string()))?;
        ...
        let reader = index
            ...
            .try_into()
            .map_err(|e: tantivy::TantivyError| InkbaseError::IndexError(e.to_string()))?;
    ...
    pub fn delete_document(&self, doc_id: &Uuid) -> inkbase_core::Result<()> {
        let mut writer = self.writer()?;
    ...
    pub fn search(
        ...
        query_str: &str,
        limit: usize,
    ) -> inkbase_core::Result<Vec<SearchResult>> {
        ...
        let query = query_parser
            ...
            .map_err(|e| InkbaseError::IndexError(format!("query parse error: {e}")))?;
    ...
    fn writer(&self) -> inkbase_core::Result<IndexWriter> {
        self.index
...
fn block_type_to_tag(bt: &inkbase_core::types::BlockType) -> &'static str {
    use inkbase_core::types::BlockType;
    match bt {
...
mod tests {
    use super::*;
    use chrono::Utc;
    use inkbase_core::types::{BlockType, DocumentRecord};
